...
class TestContainerHealth:
    ...
    def test_spartan_importable(self, running_container: str):
        """Test that spartan package is importable."""
        result = self._exec_in_container(
            running_container, "python3 -c 'import spartan; print(\"ok\")'"
        )
        assert result.returncode == 0, f"Import failed: {result.stderr}"
        assert "ok" in result.stdout, "spartan should be importable"

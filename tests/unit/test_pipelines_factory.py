...
Unit tests for spartan.core.pipelines module.
"""
...
from spartan.core.pipelines import CTF_STAGES, PENTEST_STAGES, get_stages
...
class TestCTFStages:
    ...
    def test_ctf_stage_prompts_callable(self, config: SpartanConfig):
...
class TestPentestStages:
    ...
    def test_pentest_stage_prompts_callable(self, config: SpartanConfig):
        """Test that all stage prompt builders are callable and return strings."""
        for stage in PENTEST_STAGES:
            sys_prompt = stage.get_system_prompt(config)
            assert isinstance(sys_prompt, str)

...
Unit tests for spartan.prompts.stages module.
"""
...
@pytest.fixture
def config(temp_working_dir: Path) -> SpartanConfig:
...
@pytest.fixture
def config_with_instruction(temp_working_dir: Path) -> SpartanConfig:
...
class TestBuildPriorContextBlock:
    ...
    def test_truncation(self):
        ...
        context = stages._build_prior_context_block(results)
        assert "...[TRUNCATED]" in context or "... [truncated]" in context or "[TRUNCATED" in context or "..." in context
...
class TestCTFPrompts:
    ...
    def test_ctf_stage1_system_prompt(self, config: SpartanConfig):
        ...
        assert "Spartan" in prompt

    def test_ctf_stage1_system_prompt_with_instruction(
        self, config_with_instruction: SpartanConfig
    ):
    ...
    def test_ctf_stage1_task_prompt(self, config: SpartanConfig):
        """Test CTF stage 1 task prompt."""
        task = stages.ctf_stage1_task_prompt(config, [])
        assert "10.10.11.234" in task

    def test_ctf_stage2_system_prompt(self, config: SpartanConfig):
    ...
    def test_ctf_stage2_task_prompt_without_context(self, config: SpartanConfig):
    ...
    def test_ctf_stage3_system_prompt(self, config: SpartanConfig):
...
class TestPentestPrompts:
    ...
    def test_pentest_stage1_system_prompt(self, config: SpartanConfig):
    ...
    def test_pentest_stage1_task_prompt(self, config: SpartanConfig):
        """Test Pentest stage 1 task prompt."""
        task = stages.pentest_stage1_task_prompt(config, [])
        assert "10.10.11.234" in task

    def test_pentest_stage2_system_prompt(self, config: SpartanConfig):
    ...
    def test_pentest_stage3_system_prompt(self, config: SpartanConfig):

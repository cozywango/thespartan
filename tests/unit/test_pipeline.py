...
Unit tests for spartan.core.pipeline module.
"""
...
from spartan.core.backend import AgentMessage, MessageType
from spartan.core.config import SpartanConfig
from spartan.core.events import EventBus
from spartan.core.pipeline import (
    PipelineMode,
...
def _make_stage_def(
    ...
    return StageDefinition(
        ...
        get_system_prompt=lambda config, _sp=sys_prompt: _sp,  # type: ignore
        get_task_prompt=lambda config, prior, _tp=task_prompt: _tp,  # type: ignore
    )
...
class TestPipelineOrchestrator:
    ...
    @pytest.fixture
    def config(self, temp_working_dir: Path) -> SpartanConfig:
        """Create test config."""
    ...
    async def test_run_single_stage(
        ...
        # Monkeypatch OrnithBackend to return our mock
        monkeypatch.setattr(
            "spartan.core.backend.OrnithBackend",
    ...
    async def test_stop_halts_pipeline(
        ...
        stop_backend = StopAfterFirstBackend(orchestrator)  # type: ignore

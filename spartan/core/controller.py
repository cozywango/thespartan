...
from spartan.core.backend import (
    AgentBackend,
    AgentMessage,
    MessageType,
)
...
from spartan.core.session import SessionStatus, SessionStore
...
class AgentController:
    ...
    def __init__(
        ...
        Args:
            ...
            backend: Agent backend for LLM communication
    ...
    async def run(self, task: str, resume_session_id: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            self._set_state(AgentState.IDLE, "Cancelled")
            self.sessions.update_status(SessionStatus.PAUSED)
            raise
    ...
    async def _process_message(
        ...
        if msg.type == MessageType.TEXT:
            ...
            output_parts.append(f"\nTool Call: {msg.tool_name} | Args: {msg.tool_args}")
            ...
            output_parts.append(f"--- Result from {msg.tool_name} ---\n{msg.content}")
            self.events.emit_tool(
                status="complete",
                name=msg.tool_name or "unknown",
                result=msg.content,

...
def main(file_name):
    ...
    bot_responses = logs["Spartan"]
    merged_list = [[user_input[0], user_input[1], "user"] for user_input in user_inputs]
    merged_list.extend(
        [bot_response[0], bot_response[1], "Spartan"] for bot_response in bot_responses
    )
    ...
    for element in merged_list:
        ...
        timestamp = datetime.datetime.fromtimestamp(int(element[0])).strftime("%Y-%m-%d %H:%M:%S")
        output += f"{timestamp} [{element[2]}]: {element[1]}\n"
        # add an additional line break if the element is from bot
        if element[2] == "Spartan":
            output += "----------------------------------------\n\n"
    # print the output
...
if __name__ == "__main__":
    # default filename = "../logs/sample_Spartan_log.txt"
    file_name = "logs/sample_Spartan_log.txt" if len(sys.argv) == 1 else sys.argv[1]
    main(file_name)

"""Classic Spartan orchestrator (reasoning / generation / parsing + PTT).
...
from spartan_legacy.llm.client import LLMClient
from spartan_legacy.llm.factory import get_client
from spartan_legacy.prompts.prompt_class import SpartanPrompt
from spartan_legacy.utils.prompt_select import prompt_ask, prompt_select
from spartan_legacy.utils.task_handler import (
    local_task_entry,
    localTaskCompleter,
    main_task_entry,
    mainTaskCompleter,
...
class Spartan:
    ...
    def __init__(
        ...
        logger.add(sink=os.path.join(log_dir, "Spartan.log"))
        ...
        self.console.print(
            "Welcome to Spartan (modernized legacy), an interactive penetration testing assistant.",
            style="bold green",
        )
    ...
    def _feed_init_prompts(self) -> None:
        ...
        self.console.print("Spartan output: ", style="bold green")
        self.console.print(response)
        self.log_conversation("Spartan", "Spartan output:" + response)
    ...
    def local_input_handler(self) -> str:
        ...
        if local_request_option == "help":
            ...
            self.console.print("Please share your findings and questions with Spartan.")
            self.log_conversation(
                "Spartan",
                "Please share your findings and questions with Spartan. (End with <shift + right-arrow>)",
            )
            user_input = prompt_ask("Your input: ", multiline=True)
            self.log_conversation("user", user_input)
            with self.console.status("[bold green] Spartan Thinking..."):
                local_task_response = self.test_generation_handler(
                    self.prompts.local_task_prefix + user_input
                )
            self.console.print("Spartan:\n", style="bold green")
            self.console.print(local_task_response + "\n", style="yellow")
            self.log_conversation("Spartan", local_task_response)

        elif local_request_option == "brainstorm":
            self.console.print("Please share your concerns and questions with Spartan.")
            self.log_conversation(
                "Spartan",
                "Please share your concerns and questions with Spartan. End with <shift + right-arrow>)",
            )
            user_input = prompt_ask("Your input: ", multiline=True)
            self.log_conversation("user", user_input)
            with self.console.status("[bold green] Spartan Thinking..."):
            ...
            self.log_conversation("Spartan", _GOOGLE_PLACEHOLDER)
            ...
            self.log_conversation("Spartan", "Exit the local task and continue the main task.")
    ...
    def input_handler(self) -> Any:
        ...
        if request_option == "next":
            ...
            if not self.step_reasoning_response:
                ...
                self.log_conversation("Spartan", msg)
                return msg
            self.console.print(
                "Spartan will generate more test details, and enter the sub-task "
                "generation mode. (Pressing Enter to continue)",
                style="bold green",
            )
            self.log_conversation(
                "Spartan",
                "Spartan will generate more test details, and enter the sub-task generation mode.",
            )
            input()
            with self.console.status("[bold green] Spartan Thinking..."):
            ...
            self.log_conversation("Spartan", response)
            ...
            self.console.print(
                "Please share your thoughts/questions with Spartan. (End with <shift + right-arrow>) "
            )
            self.log_conversation(
                "Spartan", "Please share your thoughts/questions with Spartan."
            )
            user_input = prompt_ask("Your input: ", multiline=True)
            self.log_conversation("user", user_input)
            with self.console.status("[bold green] Spartan Thinking..."):
            ...
            response = False
            self.console.print("Thank you for using Spartan!", style="bold green")
            self.log_conversation("Spartan", "Thank you for using Spartan!")

        else:
            self.console.print("Please key in the correct options.", style="bold red")
            self.log_conversation("Spartan", "Please key in the correct options.")
    ...
    def main(self) -> None:
        ...
        timestamp = time.time()
        log_path = os.path.join(self.log_dir, f"Spartan_log_{timestamp}.txt")
